...
def default_chain(samplerate: int = 48000, channels: int = 1) -> EffectChain:
    """Signal order: gate -> pitch correction -> compressor -> EQ -> robot voice -> delay -> reverb.

    The corrective effects (gate/compressor/EQ) are on by default; the creative
    voice effects (pitch/robot/delay/reverb) start disabled so passthrough stays
    clean until the user opts in.
    """
    return EffectChain(
        [
